...
import CommuniWriteUI from "./CommuniWrite.presenter";
import { CREATE_BOARD, UPDATE_BOARD } from "./CommuniWrite.queries";
import { ICommuniWriteProps, IUpdateBoardInput } from "./CommuniWrite.types";
import {
  IMutation,
...
export default function CommuniWrite(props: ICommuniWriteProps) {
  ...
  const onClickSubmit = async () => {
    ...
    (Object.keys(inputs) as Array<keyof typeof inputs>).forEach((el, i) => {
      if (!inputs[el]) {
        setInputsError((prev) => ({ ...prev, [el]: errors[el] }));
        console.log(el, errors[el]);
      }
    });

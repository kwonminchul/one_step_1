// import * as O from "./CommuniDetail.styles";
// // import { v4 as uuidv4 } from "uuid";
// import DOMPurify from "dompurify";

// export default function CommuniDetailUI(props: any) {
//   return (
//     <O.Wrapper>
//       <O.TopWrapper>
//         <O.ImageWrapper>
...
import * as O from "./CommuniDetail.styles";
import { ICommuniDetailUIProps } from "./CommuniDetail.types";

export default function CommuniDetailUI(props: ICommuniDetailUIProps) {
  ...
  return (
    <O.Wrapper>
      ...
      <O.Header>
        <O.AvatarWrapper>
          <O.Avatar src="/profile.png" />
          <O.Info>
            <O.Writer>{props.data?.fetchBoard?.writer}</O.Writer>
            <O.CreatedAt>
        ...
        <O.IconWrapper>
          <O.IconLink src="/ic_link-32px.png" />
          <Tooltip
            placement="top"
            title={`${props.data?.fetchBoard.boardAddress?.address} ${props.data?.fetchBoard.boardAddress?.addressDetail}`}
          >
            <O.IconLocation src="/ic_location_on-32px.png" />
          </Tooltip>
        </O.IconWrapper>
      </O.Header>
      <O.Title>{props.data?.fetchBoard?.title}</O.Title>
      <O.Contents>{props.data?.fetchBoard?.contents}</O.Contents>
      <O.Body>
        {/* <O.Title>{props.data?.fetchBoard?.title}</O.Title> */}
        <O.ImageWrapper>
          {props.data?.fetchBoard.images
            ?.filter((el: string) => el)
            .map((el: string) => (
              <O.Image key={el} src={`https://storage.googleapis.com/${el}`} />
            ))}
        </O.ImageWrapper>
        ...
        <O.LikeWrapper>
          <O.IconWrapper>
            <O.LikeIcon onClick={props.onClickLike} />
            <O.LikeNumber>{props.data?.fetchBoard.likeCount}</O.LikeNumber>
          </O.IconWrapper>
          <O.IconWrapper>
            <O.UnlikeIcon onClick={props.onClickDislike} />
            <O.UnlikeNumber>
      ...
      {/* </O.CardWrapper> */}
      <O.BottomWrapper>
        <O.Button onClick={props.onClickMoveToBoardList}>LIST</O.Button>
        <O.Button onClick={props.onClickMoveToBoardEdit}>EDIT</O.Button>
        <O.Button onClick={props.onClickDelete}>DELETE</O.Button>
      </O.BottomWrapper>
    </O.Wrapper>
  );
}
